...
function add_popups_wp_footer_PPS(){
	...
	if(count($pps_ids) > 0){
		...
		//Si ahora hay más popups quiere decir que se hay popups dentro de popups, volvemos a llamarlos.
...
function add_popup_admin_footer_PPS($popup_id = 0) {
	...
	//Compatibility for nested popups
	$pps_ids = isset($GLOBALS['pps_ids']) ? $GLOBALS['pps_ids'] : array();
	$popups = '';
	if(count($pps_ids) > 0){
		foreach ($pps_ids as $key => $popup_id) {
...
function disable_popup_PPS($popup_id){
	...
	$disable_mobile = isset($pps_data['pps_disable_mobile'][0]) ? $pps_data['pps_disable_mobile'][0] : $pps_options['prevent_mobile'];
	if ($disable_mobile == 'true') {
		if ( $deviceType == 'phone' || $deviceType == 'tablet' )
			$disable = true;
	}
	//Desactiva el Popup en Escritorios
	$disable_desktop = isset($pps_data['pps_disable_desktop'][0]) ? $pps_data['pps_disable_desktop'][0] : $pps_options['prevent_desktop'];
...
function get_iframe_content_PPS($pps_data, &$numItems){
	...
	if( !empty($iframe_content)) {
		$content = '<li><div class="pps-iframe" data-src="'.$iframe_content.'"><iframe src="about:blank" height="'.$pps_data['pps_iframe_height'][0].'"></iframe></div></li>';
		$numItems++;
	}
	return $content;
}
...
function get_pdf_content_PPS($pps_data, &$numItems){
	...
	if( !empty($pdf_content) and $filetype['ext'] == 'pdf') {
		$download_pdf = (isset($pps_data['pps_pdf_download_link'][0]) && $pps_data['pps_pdf_download_link'][0] == 'true') ? '<a href="'.$pdf_content.'" class="pps-download-pdf">Download</a>' : '';
		$protocol = get_protocol_PPS();
		$content = '<li><div class="pps-pdf"><iframe src="'.$protocol.'://docs.google.com/gview?url='.$pdf_content.'&embedded=true" style="width:100%" frameborder="0" height="'.$pps_data['pps_pdf_height'][0].'"></iframe>'.$download_pdf.'</div></li>';
		$numItems++;
	}
	return $content;
}
...
function get_disclaimer_popup_PPS($popup_id = 0){
	...
	if($disclaimer_activate == 'true' ){
		$agree_redirect_to = isset($pps_data['pps_disclaimer_agree_redirect_to'][0]) ? $pps_data['pps_disclaimer_agree_redirect_to'][0] : '#';
		$agree_redirect_to = ($agree_redirect_to == 'http://') ? '#' : $agree_redirect_to;
		$agree_redirect_target = isset($pps_data['pps_disclaimer_agree_target'][0]) ? $pps_data['pps_disclaimer_agree_target'][0] : '_self';

		$disagree_redirect_to = isset($pps_data['pps_disclaimer_disagree_redirect_to'][0]) ? $pps_data['pps_disclaimer_disagree_redirect_to'][0] : '#';
		$disagree_redirect_to = ($disagree_redirect_to == 'http://') ? '#' : $disagree_redirect_to;
		$disagree_redirect_target = isset($pps_data['pps_disclaimer_disagree_target'][0]) ? $pps_data['pps_disclaimer_disagree_target'][0] : '_self';

		$disclaimer_popup .= '<div class="pps-disclaimer">';
			$disclaimer_popup .= '<a href="'.$agree_redirect_to.'" id="pps-btn-agree-'.$popup_id.'" class="'.$pps_data['pps_disclaimer_agree_button_css'][0].'" target="'.$agree_redirect_target.'">'.$pps_data['pps_disclaimer_agree_button_text'][0].'</a>';
			$disclaimer_popup .= '<a href="'.$disagree_redirect_to.'" id="pps-btn-disagree-'.$popup_id.'" class="'.$pps_data['pps_disclaimer_disagree_button_css'][0].'" target="'.$disagree_redirect_target.'">'.$pps_data['pps_disclaimer_disagree_button_text'][0].'</a>';
