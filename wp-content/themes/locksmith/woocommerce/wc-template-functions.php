...
function wc_products_rss_feed() {
	...
	if ( is_post_type_archive( 'product' ) || is_singular( 'product' ) ) {
		...
		echo '<link rel="alternate" type="application/rss+xml"  title="' . esc_html__( 'New products', 'locksmith' ) . '" href="' . esc_attr( $feed ) . '" />';
		...
		echo '<link rel="alternate" type="application/rss+xml"  title="' . sprintf(esc_html__( 'New products added to %s', 'locksmith' ), urlencode($term->name)) . '" href="' . esc_attr( $feed ) . '" />';
		...
		echo '<link rel="alternate" type="application/rss+xml"  title="' . sprintf(esc_html__( 'New products tagged %s', 'locksmith' ), urlencode($term->name)) . '" href="' . esc_attr( $feed ) . '" />';
...
if ( ! function_exists( 'woocommerce_demo_store' ) ) {
	...
	function woocommerce_demo_store() {
		...
		if ( empty( $notice ) )
			$notice = esc_html__( 'This is a demo store for testing purposes &mdash; no orders shall be fulfilled.', 'locksmith' );
...
if ( ! function_exists( 'woocommerce_page_title' ) ) {
	...
	function woocommerce_page_title( $echo = true ) {
		...
		if ( is_search() ) {
			$page_title = sprintf( esc_html__( 'Search Results: &ldquo;%s&rdquo;', 'locksmith' ), get_search_query() );

			if ( get_query_var( 'paged' ) )
				$page_title .= sprintf( esc_html__( '&nbsp;&ndash; Page %s', 'locksmith' ), get_query_var( 'paged' ) );
...
if ( ! function_exists( 'woocommerce_catalog_ordering' ) ) {
	...
	function woocommerce_catalog_ordering() {
		...
		$catalog_orderby_options = apply_filters( 'woocommerce_catalog_orderby', array(
			'menu_order' => esc_html__( 'Default sorting', 'locksmith' ),
			'popularity' => esc_html__( 'Sort by popularity', 'locksmith' ),
			'rating'     => esc_html__( 'Sort by average rating', 'locksmith' ),
			'date'       => esc_html__( 'Sort by newness', 'locksmith' ),
			'price'      => esc_html__( 'Sort by price: low to high', 'locksmith' ),
			'price-desc' => esc_html__( 'Sort by price: high to low', 'locksmith' )
		) );
	...
	function woocommerce_default_product_tabs( $tabs = array() ) {
		...
		if ( $post->post_content ) {
			$tabs['description'] = array(
				'title'    => esc_html__( 'Description', 'locksmith' ),
				'priority' => 10,
		...
		if ( $product && ( $product->has_attributes() || ( $product->enable_dimensions_display() && ( $product->has_dimensions() || $product->has_weight() ) ) ) ) {
			$tabs['additional_information'] = array(
				'title'    => esc_html__( 'Additional Information', 'locksmith' ),
				'priority' => 20,
		...
		if ( comments_open() ) {
			$tabs['reviews'] = array(
				'title'    => sprintf( esc_html__( 'Reviews (%d)', 'locksmith' ), get_comments_number( $post->ID ) ),
				'priority' => 30,
...
if ( ! function_exists( 'woocommerce_related_products' ) ) {
	...
	function woocommerce_related_products( $args = array(), $columns = false, $orderby = false ) {
		if ( ! is_array( $args ) ) {
			_deprecated_argument( __FUNCTION__, '2.1', esc_html__( 'Use $args argument as an array instead. Deprecated argument will be removed in WC 2.2.', 'locksmith' ) );
...
if ( ! function_exists( 'woocommerce_breadcrumb' ) ) {
	...
	function woocommerce_breadcrumb( $args = array() ) {
		...
		$defaults = apply_filters( 'woocommerce_breadcrumb_defaults', array(
			...
			'before'      => '',
			'after'       => '',
			'home'        => _x( 'Home', 'breadcrumb', 'locksmith' ),
		) );
...
if ( ! function_exists( 'woocommerce_order_again_button' ) ) {
	...
	function woocommerce_order_again_button( $order ) {
		...
		?>
		<p class="order-again">
			<a href="<?php echo wp_nonce_url( add_query_arg( 'order_again', $order->id ) , 'woocommerce-order_again' ); ?>" class="button"><?php esc_html__( 'Order Again', 'locksmith' ); ?></a>
		</p>
		<?php
	}
}
...
if ( ! function_exists( 'woocommerce_form_field' ) ) {
	...
	function woocommerce_form_field( $key, $args, $value = null ) {
		...
		if ( $args['required'] ) {
			...
			$required = ' <abbr class="required" title="' . esc_attresc_html__( 'required', 'locksmith'  ) . '">*</abbr>';
		} else {
			$required = '';
		}
		...
		case "country" :
			...
			if ( sizeof( $countries ) == 1 ) {
				...
				$field = '<p class="form-row ' . esc_attr( implode( ' ', $args['class'] ) ) .'" id="' . esc_attr( $args['id'] ) . '_field">'
						...
						. '<option value="">'.esc_html__( 'Select a country&hellip;', 'locksmith' ) .'</option>';
				...
				$field .= '<noscript><input type="submit" name="woocommerce_checkout_update_totals" value="' . esc_html__( 'Update country', 'locksmith' ) . '" /></noscript>';
		...
		case "state" :
			...
			if ( is_array( $states ) && empty( $states ) ) {
				...
				$field .= '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="state_select" ' . implode( ' ', $custom_attributes ) . ' placeholder="' . esc_attr( $args['placeholder'] ) . '">
					<option value="">'.esc_html__( 'Select a state&hellip;', 'locksmith' ) .'</option>';
...
if ( ! function_exists( 'get_product_search_form' ) ) {
	...
	function get_product_search_form( $echo = true  ) {
		...
		$form = '<form role="search" method="get" id="searchform" action="' . esc_url( home_url( '/'  ) ) . '">
			<div>
				<label class="screen-reader-text" for="s">' . esc_html__( 'Search for:', 'locksmith' ) . '</label>
				<input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="' . esc_html__( 'Search for products', 'locksmith' ) . '" />
				<input type="submit" id="searchsubmit" value="'. esc_attresc_html__( 'Search', 'locksmith' ) .'" />
