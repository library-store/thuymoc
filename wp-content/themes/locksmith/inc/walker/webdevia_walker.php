 ...
 * ---------------menu--------------------------------
 */ 
class locksmith_top_bar_walker extends Walker_Nav_Menu {
	...
	function start_el(&$output, $item, $depth = 0, $args = Array(), $id = 0) {
	  ...
	  if(is_object($args)){
		  $locksmith_class = '';
			...
			$locksmith_icon = $item->mega_menu_icon ;
		if($item->mega_menu == 1) {
			$locksmith_class= 'wd_mega-menu';
		}
			...
			$class_names .= ($args -> has_children) ? 'has-dropdown not-click '.$locksmith_class : '';
			...
			<a' . $attributes . '>' : '';
			if(($locksmith_icon != '') and ($locksmith_icon != '---- None ----'))  {
				
			$item_output .= '<i class="'.$locksmith_icon.' fa"></i> ';
			}
...
if ( !function_exists( 'locksmith_add_mega_menu_support' ) ):

	function locksmith_add_mega_menu_support() {
			...
			add_filter( 'wp_setup_nav_menu_item', 'locksmith_add_custom_nav_fields' );

			add_action( 'wp_update_nav_menu_item', 'locksmith_update_custom_nav_fields', 10, 3 );

			add_filter( 'wp_edit_nav_menu_walker', 'locksmith_edit_menu_walker', 10, 2 );

			//add_filter( 'nav_menu_css_class', 'greenenergy_add_class_to_menu', 10, 2 );
...
add_action( 'init', 'locksmith_add_mega_menu_support' );



/* Add custom fields to menu */

if ( !function_exists( 'locksmith_add_custom_nav_fields' ) ):

	function locksmith_add_custom_nav_fields( $menu_item ) {

		
		$menu_item->mega_menu_icon = get_post_meta( $menu_item->ID, '_locksmith_mega_menu_icon', true )? get_post_meta( $menu_item->ID, '_locksmith_mega_menu_icon', true ) : '';
		
		$menu_item->mega_menu_bg_image = get_post_meta( $menu_item->ID, '_locksmith_mega_menu_bg_image', true )? get_post_meta( $menu_item->ID, '_locksmith_mega_menu_bg_image', true ) : '';
		
		$menu_item->mega_menu = get_post_meta( $menu_item->ID, '_locksmith_mega_menu', true ) ? 1 : 0;
...
if ( !function_exists( 'locksmith_update_custom_nav_fields' ) ):

	function locksmith_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {


			$value = isset($_REQUEST['menu-item-mega-menu-icon'][$menu_item_db_id])? $_REQUEST['menu-item-mega-menu-icon'][$menu_item_db_id] : '' ;
			update_post_meta( $menu_item_db_id, '_locksmith_mega_menu_icon', $value );
			
			
			$value = isset($_REQUEST['menu-item-mega-menu-bg-image'][$menu_item_db_id])? $_REQUEST['menu-item-mega-menu-bg-image'][$menu_item_db_id] : '' ;
			update_post_meta( $menu_item_db_id, '_locksmith_mega_menu_bg_image', $value );
		

			$value = isset( $_REQUEST['menu-item-mega-menu'][$menu_item_db_id] ) ? 1 : 0;
			update_post_meta( $menu_item_db_id, '_locksmith_mega_menu', $value );
...
if ( !function_exists( 'locksmith_edit_menu_walker' ) ):

	function locksmith_edit_menu_walker( $walker, $menu_id ) {



		class locksmith_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit {
				...
				$mega_menu_html = '';
  				...
  				$icons = unserialize( $icons );
		        	...
		        	$mega_menu_html .= '</select>
		                ...
		                '.__( 'display icone', 'locksmith' ).'</label>
					...
					$mega_menu_html .= '<p class="field-custom description description-wide">
			                ...
			                '.__( 'Mega Menu (Background image)', 'locksmith' ).'
					...
					$mega_menu_html .= '<p class="field-custom description description-wide">
		                ...
		                '.__( 'Mega Menu (classic)', 'locksmith' ).'
		...
		return 'locksmith_Walker_Nav_Menu_Edit';
